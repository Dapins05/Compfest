"""Konversi MVTec AD menjadi SampleRecord.
...
def convert_category(
    ...
    *,
    ...
    """Ubah satu kategori MVTec menjadi record cacat dan record normal."""
    root = spec.raw_dir(raw_root)
    if not root.is_dir():
        raise FileNotFoundError(f"folder MVTec tidak ditemukan: {root}")
    ...
    for subset in ("train", "test"):
        ...
        for image_path in sorted(good_dir.glob("*.png")):
            normals.append(
                SampleRecord(
                    sample_id=f"mvtec_{spec.name}_{subset}_good_{image_path.stem}",
                    category=spec.name,
                    source=spec.source,
    ...
    for defect_dir in sorted((root / "test").iterdir()):
        ...
        for image_path in sorted(defect_dir.glob("*.png")):
            ...
            defects.append(
                SampleRecord(
                    sample_id=f"mvtec_{spec.name}_{defect_dir.name}_{image_path.stem}",
                    category=spec.name,
                    source=spec.source,
                    source_image=image_path,
